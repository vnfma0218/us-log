...
async function renderSlideshow({
  ...
  const perImage = Math.max(durationSec / files.length, 1)
  ...
  const listPath = join(tmpdir(), `us-log-concat-${Date.now()}.txt`)
...
async function processSingleJob(job: VideoJob) {
  ...
  try {
    ...
    for (let index = 0; index < photos.length; index += 1) {
      ...
      const localPath = join(tempDir, `${String(index + 1).padStart(3, "0")}.jpg`)
      await downloadPhoto(photo.storage_path, localPath)
